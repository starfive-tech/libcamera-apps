/**
  ...
  * @version  V1.0
  * @date  06/21/2022
  * @brief StarFive ISP tuning server tuning base functions
  ******************************************************************************
  * @copy
  *
...
//static std::thread g_tuning_task_threads[TUNING_SERVER_MAX_CONNECTION];
static std::unordered_map<SOCKET, std::thread> g_socket_thread_map;
static std::list<SOCKET> g_complete_threads;
static std::mutex g_thread_mutex;

/*-----------------------------------------------------------*/
//std::thread &get_tuning_task_thread()
//{
//	if(g_tuning_task_threads[g_client_count].joinable())
//		g_tuning_task_threads[g_client_count].join();
//	return g_tuning_task_threads[g_client_count];
//}
std::thread &get_new_tuning_thread_obj(SOCKET socket)
{
	auto it = g_socket_thread_map.find(socket);
	if(it != g_socket_thread_map.end())
		(it->second).join();
	return g_socket_thread_map[socket];
}

void join_tuning_task_thread(SOCKET socket)
{
	auto it = g_socket_thread_map.find(socket);
	if(it != g_socket_thread_map.end()) {
		(it->second).join();
		g_socket_thread_map.erase(it);
	}
}

void join_tuning_task_threads()
{
	for(auto &it : g_socket_thread_map)
		(it.second).join();
}
...
void complete_tuning_thread(SOCKET socket)
{
	...
	g_complete_threads.push_back(socket);
}

void join_complete_tuning_threads()
{
	std::lock_guard<std::mutex> lock(g_thread_mutex);
	for(SOCKET s : g_complete_threads) {
		join_tuning_task_thread(s);
	}
	g_complete_threads.clear();
}
...
int32_t tuning_base_send_command(SOCKET sock, uint32_t mod, uint32_t dir, uint32_t type, uint32_t datasize, UTVALUE val)
{
	STCOMD comd;
	comd.u32Cmd = mod;
	comd.utPara.cmdPar.u1Dir = dir;
	comd.utPara.cmdPar.u3Type = type;
	comd.utPara.cmdPar.u4GroupIdx = 0;
